...
function challengeDebugEnabled() {
  return String(process.env.CHALLENGE_DEBUG || 'true').toLowerCase() !== 'false';
}

function writeIntegrationDebug(event, payload = {}) {
  try {
    console.log(`[STARCLUB_INTEGRATION_DEBUG] ${event}`, JSON.stringify(payload));
  } catch {
    console.log(`[STARCLUB_INTEGRATION_DEBUG] ${event}`, payload);
  }
}
...
function itemOrGroupMatches(expected, item = {}) {
  const wanted = normalizeMatchValue(expected);
  if (!wanted) return true;
  const externalId = normalizeMatchValue(item.external_product_id || item.product_id);
  if (externalId && externalId === wanted) return true;
  return categoryMatches(expected, item);
}

function offerMatchesItem(offer, item) {
  const targets = [offer.product_external_id, offer.category].filter(Boolean);
  if (!targets.length) return true;
  return targets.some((target) => itemOrGroupMatches(target, item));
}
...
function addChallengeVisit(clientId, receiptId, purchasedAt, totalCents, eligibleItems, storeId = '') {
  const diagnostics = [];
  ...
  for (const ch of challenges) {
    ...
    if (ch.store_id && ch.store_id !== 'all') {
      ...
      diagnostic.receipt_store_id = storeId || '';
      if (normalizeMatchValue(ch.store_id) !== normalizeMatchValue(storeId)) {
        diagnostic.reason = 'STORE_NOT_MATCHED';
        diagnostics.push(diagnostic);
        writeChallengeDebug(diagnostic);
        continue;
      }
    ...
    if (ch.category) {
      const hasCategory = eligibleItems.some((item) =>
        itemOrGroupMatches(ch.category, item) ||
...
function updateStampProgress(clientId, receiptId, items = []) {
  ...
  for (const program of programs) {
    const count = items.reduce((sum, item) => {
      if (!itemOrGroupMatches(program.category, item)) return sum;
      return sum + Math.ceil(Number(item.qty || 1));
    }, 0);
    writeIntegrationDebug('STAMP_CHECK', {
      program_id: program.id,
      program_name: program.name,
      expected_category: program.category,
      matched_qty: count,
      receipt_id: receiptId,
    ...
    const currentRow = db.prepare('SELECT * FROM client_stamp_progress WHERE client_id = ? AND program_id = ?').get(clientId, program.id);
    const current = currentRow?.progress || 0;
    let next = current + count;
    let completed = currentRow?.completed_count || 0;
...
app.get('/api/public/stores', (req, res) => {
  const stores = db.prepare('SELECT * FROM stores WHERE is_active = 1 ORDER BY name').all();
...
app.post('/api/1c/calculate', oneCAuth, (req, res) => {
  ...
  writeIntegrationDebug('CALCULATE', {
    ...
    request_items: items.map(item => ({
      external_product_id: item.external_product_id || item.product_id || '',
      name: item.name || '',
      category_candidates: itemCategoryCandidates(item),
      qty: item.qty,
...
app.post('/api/1c/receipts', oneCAuth, (req, res) => {
  const body = req.body || {};
  writeIntegrationDebug('RECEIPT_INCOMING', {
    id: body.id || '',
    card_number: body.card_number || '',
    store_id: body.store_id || '',
    ...
    items: Array.isArray(body.items) ? body.items.map(item => ({
      ...
      parent_group_code: item.parent_group_code || '',
  ...
  if (!isRewardReceipt) {
    challengeDiagnostics = addChallengeVisit(client.id, body.id, purchasedAt, totalCents, items, body.store_id || '');
    writeIntegrationDebug('CHALLENGE_RESULT', { receipt_id: body.id, client_id: client.id, diagnostics: challengeDiagnostics });
...
app.get('/api/admin/stores', adminAuth, (req, res) => {
  const stores = db.prepare('SELECT * FROM stores ORDER BY is_active DESC, name').all();
  res.json({ ok: true, stores });
});
...
app.post('/api/admin/stores', adminAuth, (req, res) => {
  ...
  if (!id || !name) return res.status(400).json({ ok: false, error: 'STORE_ID_AND_NAME_REQUIRED', message: 'Вкажіть ID і назву магазину' });
  if (!/^[A-Za-z0-9_-]+$/.test(id)) return res.status(400).json({ ok: false, error: 'STORE_ID_INVALID', message: 'ID може містити латинські літери, цифри, дефіс і підкреслення' });
  const now = nowIso();
  try {
    db.prepare(`INSERT INTO stores(id, name, address, work_hours, phone, image_url, is_active, created_at, updated_at) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(id, name, b.address || null, b.work_hours || null, b.phone || null, b.image_url || '/assets/star.svg', b.is_active === false ? 0 : 1, now, now);
  } catch (error) {
    return res.status(409).json({ ok: false, error: 'STORE_ID_EXISTS', message: 'Магазин із таким ID уже існує' });
  }
  logAudit({ actorType: 'admin', actorId: String(req.admin.id), action: 'store_created', entityType: 'store', entityId: id, payload: b });
  res.json({ ok: true, store: db.prepare('SELECT * FROM stores WHERE id = ?').get(id) });
});

app.patch('/api/admin/stores/:id', adminAuth, (req, res) => {
  const current = db.prepare('SELECT * FROM stores WHERE id = ?').get(req.params.id);
  if (!current) return res.status(404).json({ ok: false, error: 'STORE_NOT_FOUND' });
  const b = req.body || {};
  const name = String(b.name ?? current.name).trim();
  if (!name) return res.status(400).json({ ok: false, error: 'STORE_NAME_REQUIRED', message: 'Вкажіть назву магазину' });
  db.prepare(`UPDATE stores SET name = ?, address = ?, work_hours = ?, phone = ?, image_url = ?, is_active = ?, updated_at = ? WHERE id = ?`)
    .run(name, b.address ?? current.address, b.work_hours ?? current.work_hours, b.phone ?? current.phone, b.image_url ?? current.image_url, b.is_active === undefined ? current.is_active : (b.is_active ? 1 : 0), nowIso(), req.params.id);
  ...
  res.json({ ok: true, store: db.prepare('SELECT * FROM stores WHERE id = ?').get(req.params.id) });
});
...
app.delete('/api/admin/stores/:id', adminAuth, (req, res) => {
  ...
  if (!current) return res.status(404).json({ ok: false, error: 'STORE_NOT_FOUND' });
  const refs = [
    db.prepare('SELECT COUNT(*) AS c FROM clients WHERE favorite_store = ?').get(req.params.id).c,
    db.prepare('SELECT COUNT(*) AS c FROM reward_products WHERE store_id = ?').get(req.params.id).c,
    db.prepare('SELECT COUNT(*) AS c FROM offers WHERE store_id = ?').get(req.params.id).c,
    db.prepare('SELECT COUNT(*) AS c FROM challenges WHERE store_id = ?').get(req.params.id).c,
    db.prepare('SELECT COUNT(*) AS c FROM receipts WHERE store_id = ?').get(req.params.id).c
  ].reduce((a,b)=>a+Number(b||0),0);
  if (refs > 0) return res.status(409).json({ ok: false, error: 'STORE_IN_USE', message: 'Магазин уже використовується. Вимкніть його замість видалення.' });
  db.prepare('DELETE FROM stores WHERE id = ?').run(req.params.id);
  logAudit({ actorType: 'admin', actorId: String(req.admin.id), action: 'store_deleted', entityType: 'store', entityId: req.params.id, payload: {} });
  res.json({ ok: true });
});
...
app.post('/api/admin/catalog/offers', adminAuth, (req, res) => {
  ...
  const row = {
    ...
    image_url: b.image_url || '/assets/star.svg',
    product_external_id: b.target_ref || b.product_external_id || null,
    category: b.target_ref || b.category || null,
  ...
  writeIntegrationDebug('ADMIN_OFFER_CREATE', row);
...
app.patch('/api/admin/catalog/offers/:id', adminAuth, (req, res) => {
  ...
  db.prepare(`UPDATE offers SET type=?, name=?, description=?, image_url=?, product_external_id=?, category=?, club_price_cents=?, old_price_cents=?, stars_multiplier=?, tiers_json=?, store_id=?, audience=?, active_from=?, active_to=?, is_active=?, updated_at=? WHERE id=?`)
    .run(b.type ?? existing.type, b.name ?? existing.name, b.description ?? existing.description, b.image_url ?? existing.image_url, b.target_ref ?? b.product_external_id ?? existing.product_external_id, b.target_ref ?? b.category ?? existing.category, b.club_price_cents === undefined ? existing.club_price_cents : b.club_price_cents, b.old_price_cents === undefined ? existing.old_price_cents : b.old_price_cents, b.stars_multiplier === undefined ? existing.stars_multiplier : b.stars_multiplier, b.tiers ? JSON.stringify(b.tiers) : (b.tiers_json === undefined ? existing.tiers_json : b.tiers_json), b.store_id ?? existing.store_id, b.audience ?? existing.audience, b.active_from ?? existing.active_from, b.active_to ?? existing.active_to, b.is_active === undefined ? existing.is_active : (b.is_active ? 1 : 0), nowIso(), req.params.id);
