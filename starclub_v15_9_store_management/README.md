...
Додано керування магазинами через адмін-панель: створення, редагування, вимкнення та безпечне видалення. Клієнтський Mini App автоматично показує актуальний список магазинів із бази.

## v15.6 — вкладені групи 1С

1С передає повний шлях батьківських папок товару. Накопичувальні програми, челенджі та пропозиції тепер враховують товари з усіх вкладених підгруп. Технічний код накопичувальної програми створюється автоматично.

> STAR CLUB BUILD: v15.3-challenge-category-match
...
Клієнти створюються реально через реєстрацію в додатку або через Telegram-вхід. Для перевірки 1С використовуйте номер картки, який показано у розділі “Моя карта”.
...
## v14: автоматичні клубні та оптові ціни

Додано попередній розрахунок чека через `POST /api/1c/calculate`. 1С може до оплати автоматично застосувати найнижчу клубну/оптову ціну та показати майбутнє нарахування зірок. Інструкція: `docs/1c-auto-offers-v14.md`.

## v15.5
Виправлено мобільні пропозиції, необмежені оптові рівні, збереження групи в челенджі та постійний integration debug у Railway.

## v15.7 — товар або група одним полем
- один код у формах може бути кодом товару або групи;
- групи працюють з усіма вкладеними папками;
- виправлено створення пропозицій «Множник зірок»;
- додано зрозумілі повідомлення помилок при збереженні.
