...
## v15: RMK / fiscal receipts / returns

- Star Club is now intended to be called from the real cashier workplace (RMK), not only from the manual CheckKMM form.
- Ordinary receipts without a loyalty card are allowed and skipped by loyalty processing.
- Reward items use a configurable technical fiscal price (default 0.10 UAH).
- Returns reverse previously accrued stars, including proportional partial returns.
- See `docs/1c-rmk-fiscal-returns-v15.md` and `integration-1c/RMKStarClubPatch.bsl`.
